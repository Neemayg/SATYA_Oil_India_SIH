...
* **Approved Governance Directive:**
  ...
  > Empirical system validation executed across 7 categories and documented in `docs/10-testing/system-test-benchmark-report.md`. Evaluation over frozen dev set (\(N_{\text{total}} = 62, N_{\text{events}} = 80, N_{\text{matchable}} = 63, N_{\text{accepted}} = 31\)): Layer 1 Event Extraction Recall 1.000 (62/62 clean records); Layer 2 Schedule Matching Accepted-Match Precision 100.0% (31/31, 0 false matches accepted at \(\theta \ge 0.80\)), Match Recall 49.2% (31/63 matchable events, 49 low-confidence events delegated to HITL review); Layer 3 Trust Coverage 100.0% (63/63 events, Trust Accuracy N/A); Layer 4 Schedule Projection Coverage 100.0% (60/60 activities, Forecast Accuracy N/A); Layer 5 Time Agent 19 warning signals detected (Warning Precision/Recall N/A). Confidence threshold sweep (\(\theta \in [0.40, 0.95]\)): Accepted-Match Precision = 100.0% for \(\theta \ge 0.60\); Expected Calibration Error (ECE) = 0.1783 (`CALIBRATION BASELINE / NEEDS IMPROVEMENT`, 10-bin equal-width, bimodal score distribution at 1.0 and \(\le 0.50\)). Workload benchmarks: Small p50 ~3.12 ms, Medium p50 ~3.18 ms, Large p50 ~3.24 ms (~309 ev/s throughput); 5-stage latency breakdown logged. Concurrency \(N=2, 5, 10\): exactly 1 winner per race, DB state invariants hold. Safety mutations (Rule 5 vocab guard, `STALE_REVIEW_STATE` lock, `MatchResult` immutability) and 7 programmatic property invariants (including Versioned Trust State Integrity and Project Isolation) confirmed. All 134 automated unit and integration tests pass 100% cleanly.  
  >  
